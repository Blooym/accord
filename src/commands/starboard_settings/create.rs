...
pub async fn create_cmd(
    ...
    channel: Channel,
    #[description = "The amount of reactions needed to trigger the starboard"]
    #[min = 1]
    threshold: u32,
    #[description = "The emoji to use for counting stars"] emoji: String,
    #[description = "Allow users to 'star' their own messages"] allow_selfstar: Option<bool>,
) -> Result<(), Error> {
    ...
    // Ensure that the channel is not in use.
    ...
    query!(
        "INSERT INTO guilds (id) VALUES (?1) ON CONFLICT (id) DO UPDATE SET id = ?1",
        guild_id
    )
